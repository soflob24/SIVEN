...
class Clientes extends Controller{
    ...
    public function listar(){
        ...
        for ($i=0; $i < count($data); $i++) {
            if ($data[$i]['estado'] == 1) {
                ...
                $data[$i]['acciones'] = '<div style="width: 90px;">
    ...
    public function registrar()
    {
        ...
        if (!empty($verificar) || $id_user == 1) {
            ...
            $id = $_POST['id'];
            if (empty($documento) || empty($nombre) || empty($telefono) || empty($direccion)) {
                $msg = array('msg' => 'Todo los campos son obligatorios', 'icono' => 'warning');
            }else{
                if ($id == "") {
                    $data = $this->model->registrarCliente($documento, $nombre, $telefono, $direccion);
                    if ($data == "ok") {
                    ...
                    $data = $this->model->modificarCliente($documento, $nombre, $telefono, $direccion, $id);
                    if ($data == "modificado") {
    ...
    public function eliminar(int $id)
    {
        ...
        if ($data == 1) {
            $msg = array('msg' => 'Cliente eliminado exitosamente', 'icono' => 'success');
        }else{
